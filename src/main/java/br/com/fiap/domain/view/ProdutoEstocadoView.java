...
public class ProdutoEstocadoView {
    ...
    public List<Deposito> depositos = new ArrayList<>();
    public List<ProdutoEstocado> produtosEstocados = new ArrayList<>();
    ...
    public List<ProdutoEstocado> formEstocar() {
        ...
        if (produtos.size() > 0 && depositos.size() > 0) {
            ...
            do {
                quantidade = Integer.valueOf(JOptionPane.showInputDialog("Informe a quantidade de itens do produto " + produto.getNome() + " que deseja encaminhar para estoque!"));
            } while (quantidade <= 0);
            ...
            //produtosEstocados = ProdutoEstocadoService.entrada(pe);


            if (produtosEstocados.size() > 0)
                JOptionPane.showMessageDialog(null, "Sucesso!\n" + produtosEstocados.size() + " itens de " + produto.getNome() + " produtos estocados com sucesso");
